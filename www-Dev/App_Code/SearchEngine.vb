...
Namespace BridgesInterface
    Public Class SearchEngine
        ...
        Public Sub FindTicketByPartInvoiceNumber(ByVal strPartInvoiceNumber As String, ByRef dgv As DataGrid)
            Dim cnn As New SqlConnection(Me._ConnectionString)
            Dim da As New SqlDataAdapter
            Dim ds As New DataSet
            cnn.Open()
            Dim cmd As New SqlCommand("spFindTicketByPartInvoiceNumber")
            cmd.CommandType = CommandType.StoredProcedure
            cmd.Parameters.Add("@PartInvoiceNumber", SqlDbType.VarChar, Me.TrimTrunc(strPartInvoiceNumber, &H80).Trim.Length).Value = Me.TrimTrunc(strPartInvoiceNumber, &HFF).Trim
            cmd.Connection = cnn
            da.SelectCommand = cmd
            da.Fill(ds)
            dgv.DataSource = ds
        ...
        Public Sub FindTicketByPartRMANumber(ByVal strPartRMANumber As String, ByRef dgv As DataGrid)
            Dim cnn As New SqlConnection(Me._ConnectionString)
            Dim da As New SqlDataAdapter
            Dim ds As New DataSet
            cnn.Open()
            Dim cmd As New SqlCommand("spFindTicketByPartRMANumber")
            cmd.CommandType = CommandType.StoredProcedure
            cmd.Parameters.Add("@PartRMANumber", SqlDbType.VarChar, Me.TrimTrunc(strPartRMANumber, &H80).Trim.Length).Value = Me.TrimTrunc(strPartRMANumber, &HFF).Trim
            cmd.Connection = cnn
            da.SelectCommand = cmd
            da.Fill(ds)
            dgv.DataSource = ds
