...
Namespace BridgesInterface
    Public Class PartnerAgentRequirementRecord
...
#Region "Public Sub-Routines"
        ...
        ''' </summary>
        ''' <param name="lngPartnerAgentRequiremntID">The primary key of the record you wish to load</param>
        ''' <param name="strConnectionString">The connection string to the database</param>
        ''' <remarks></remarks>
        Public Sub New(ByVal lngPartnerAgentRequiremntID As Long, ByVal strConnectionString As String)
            _ConnectionString = strConnectionString
            Load(_PartnerAgentRequiremntID)
        End Sub

        ''' <summary>
        '''  Adds a new PartnerAgentRequirement record to the database.
        ''' </summary>
        ...
        Public Sub Add(ByVal lngPartnerAgentID As Long, ByVal lngRequirementID As Long)
            If _ConnectionString.Trim.Length > 0 Then
                Dim cnn As New SqlClient.SqlConnection(_ConnectionString)
                Dim cmd As New SqlClient.SqlCommand("spAddPartnerAgentRequirement")
                cmd.CommandType = CommandType.StoredProcedure
                Dim lngPartnerAgentRequiremntID As Long = 0
                ...
                cnn.Open()
                cmd.Connection = cnn
                lngPartnerAgentRequiremntID = CType(cmd.ExecuteScalar, Long)
                cnn.Close()
                If lngPartnerAgentRequiremntID > 0 Then
                    Load(lngPartnerAgentRequiremntID)
                End If
            End If
        End Sub

        ''' <summary>
        ''' Loads a PartnerAgentRequirement record by its primary key
        ''' </summary>
        Public Sub Load(ByVal lngPartnerAgentRequiremntID As Long)
            If _ConnectionString.Trim.Length > 0 Then
                Dim cnn As New SqlClient.SqlConnection(_ConnectionString)
                Dim cmd As New SqlClient.SqlCommand("spGetPartnerAgentRequirement")
                Dim dtr As SqlClient.SqlDataReader
                cmd.CommandType = CommandType.StoredProcedure
                cmd.Parameters.Add("@PartnerAgentRequiremntID", SqlDbType.Int).Value = lngPartnerAgentRequiremntID
                cnn.Open()
                cmd.Connection = cnn
                dtr = cmd.ExecuteReader
                If dtr.Read Then
                    _PartnerAgentRequiremntID = CType(dtr("PartnerAgentRequiremntID"), Long)
        ...
        Public Sub Save(ByRef strChangeLog As String)
            ...
            If _ConnectionString.Trim.Length > 0 Then
                ...
                Dim cnn As New SqlClient.SqlConnection(_ConnectionString)
                cnn.Open()
                Dim obj As New PartnerAgentRequirementRecord(_PartnerAgentRequiremntID, _ConnectionString)
                obj.load(_PartnerAgentRequiremntID)
        ...
        ''' <summary>
        ''' Deletes the currently loaded PartnerAgentRequirement Record
        ''' </summary>
        Public Sub Delete()
            If _ConnectionString.Trim.Length > 0 Then
                Dim cnn As New SqlClient.SqlConnection(_ConnectionString)
                Dim cmd As New SqlClient.SqlCommand("spRemovePartnerAgentRequirement")
                cmd.CommandType = CommandType.StoredProcedure
                cmd.Parameters.Add("@PartnerAgentRequiremntID", SqlDbType.Int).Value = _PartnerAgentRequiremntID
                cnn.Open()
                cmd.Connection = cnn
                cmd.ExecuteNonQuery()
                cnn.Close()
...
#Region "ReadOnly Properties"
        ...
        ''' <summary>
        ''' Returns the PartnerAgentRequiremntID field for the currently loaded record
        ''' </summary>
        Public ReadOnly Property PartnerAgentRequiremntID() As Long
            Get
                Return _PartnerAgentRequiremntID
            End Get
        End Property
...
#Region "Private Sub-Routines"
        ...
        Private Sub UpdatePartnerAgentID(ByVal NewPartnerAgentID As Long, ByRef cnn As SqlClient.sqlConnection)
            Dim cmd As New sqlClient.sqlCommand("spUpdatePartnerAgentRequirementPartnerAgentID")
            cmd.CommandType = CommandType.StoredProcedure
            cmd.parameters.Add("@PartnerAgentRequiremntID", sqlDBType.int).value = _PartnerAgentRequiremntID
        ...
        Private Sub UpdateRequirementID(ByVal NewRequirementID As Long, ByRef cnn As SqlClient.sqlConnection)
            Dim cmd As New sqlClient.sqlCommand("spUpdatePartnerAgentRequirementRequirementID")
            cmd.CommandType = CommandType.StoredProcedure
            cmd.parameters.Add("@PartnerAgentRequiremntID", sqlDBType.int).value = _PartnerAgentRequiremntID
        ...
        Private Sub UpdateExpirationDate(ByVal NewExpirationDate As DateTime, ByRef cnn As SqlClient.SqlConnection)
            Dim cmd As New SqlClient.SqlCommand("spUpdatePartnerAgentRequirementExpirationDate")
            cmd.CommandType = CommandType.StoredProcedure
            cmd.Parameters.Add("@PartnerAgentRequiremntID", SqlDbType.Int).Value = _PartnerAgentRequiremntID
...
#Region "Private Members"
        ...
        Private _PartnerAgentRequiremntID As Long = 0
