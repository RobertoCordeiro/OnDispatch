...
<script runat="server"> 
  ...
  Private Sub LoadResumeType()
        Dim lay As New BridgesInterface.LayerResumeTypeAssignmentRecord(System.Configuration.ConfigurationManager.AppSettings("DBcnn"))
        lay.Load(_ID)
        Dim rtr As New BridgesInterface.ResumeTypeRecord(lay.ConnectionString)
        rtr.Load(lay.ResumeTypeID)
    lblResumeType.Text = "<div><pre>" & rtr.ResumeType  & "</pre></div>"
  End Sub
    
  Private Sub btnOK_Click(ByVal S As Object, ByVal E As EventArgs)
        Dim par As New BridgesInterface.LayerResumeTypeAssignmentRecord(System.Configuration.ConfigurationManager.AppSettings("DBCnn"))
    par.Load(_ID)
    par.Delete()
