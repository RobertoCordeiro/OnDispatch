...
<script runat="server">  
  ...
  Private Sub Page_Load(ByVal s As Object, ByVal E As EventArgs)
    If User.Identity.IsAuthenticated Then
      ...
      Master.PageHeaderText = " Upload Partner Agent Documents"
      Master.PageTitleText = " Upload Partner Agent Documents"
    End If
    Try
  ...
  Private Sub btnUpload_Click(ByVal S As Object, ByVal E As EventArgs)
    ...
    If IsComplete() Then
      ...
      Dim pdr As New BridgesInterface.PartnerAgentDocumentRecord(System.Configuration.ConfigurationManager.AppSettings("DBCnn"))
      ...
      par.Load(_ID)
         ...
         rnt.Add(CType(ptr.ResumeID, Long), Master.UserID, "Auto Note: New Document has been attached to the Partner Agent's Account: (" & par.FirstName & " " & par.LastName & ")- " & drpDocTypes.SelectedItem.ToString())
        ...
        Case "doce"
         If _FileID <> 0 then
           If _DocID <> 0 then
              ...
              pdr.Load (_DocID)
              pdr.PartnerDocumentTypeID = Ctype(drpDocTypes.SelectedValue,Long)
              pdr.FileID= lng
    ...
    If strPath.Trim.Length > 0 Then
      ...
      If IsAcceptableType(strExtension) Then
        If fup.PostedFile.ContentLength > 0 Then
          ...
          Dim pdr As New BridgesInterface.PartnerAgentDocumentRecord(System.Configuration.ConfigurationManager.AppSettings("DBCnn"))
          pdr.Add (2, _ID, drpDocTypes.SelectedValue,fil.FileID )
          'Dim act As New BridgesInterface.ActionRecord(System.Configuration.ConfigurationManager.AppSettings("DBCnn"))
          'Dim strIp As String = Request.QueryString("REMOTE_ADDR")
          'Dim strType As String = Request.ServerVariables("HTTP_USER_AGENT")
          'If IsNothing(strIp) Then
          ...
          '  strType = "web"
          'End If
          'act.Add(2, "web", strType, strIp, "web", 23, rsm.PartnerAgentDocumentID, strChangeLog)
          'LoadDocuments(_ID)
          Return fil.FileID 
        Else
  ...
  Private Function UpdateUploadFile(lngFileID as long) As Long
    ...
    ptr.Load(par.PartnerID)
    Dim rnt As New BridgesInterface.ResumeNoteRecord(System.Configuration.ConfigurationManager.AppSettings("DBCnn"))
    rnt.Add(CType(ptr.ResumeID, Long), Master.UserID, "Auto Note: A Document has been updated in the Partner Agent's Account: (" & par.FirstName & " " & par.LastName & ")- " & drpDocTypes.SelectedItem.ToString()  )
  ...
  Private Sub LoadDocTypes()
    ...
    ldr.LoadSimpleDropDownList("spGetPartnerDocumentTypes","Description","PartnerDocumentTypeID",drpDocTypes)
