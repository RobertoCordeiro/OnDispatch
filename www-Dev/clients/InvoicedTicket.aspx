<%@ Page Language="VB" masterpagefile="~/masters/cust.master" ValidateRequest="false"%>
...
<script language="VB" runat="server">
  ...
  Private Sub Secure()
    ...
    Else
      If IsDBNull (tkt.WebInvoiceID) or (tkt.WebInvoiceID = 0) then
        _ReadOnly = false
      else
  ...
  Private Sub LoadTicket()
    ...
    dectotalCharge = GetTotalCharge(tkt.TicketID)
    GetStatuses (tkt.TicketID)
  End Sub
...
Private Sub btnAdd_Click(ByVal S As Object, ByVal E As EventArgs)
  ...
  Private Sub LoadAttachedDocuments(ByVal lngTicketID As Long)
        ...
        ldr.LoadSingleLongParameterDataGrid("spGetTicketDocumentsForCustomers", "@TicketID", lngTicketID, dgvAttachments)
    End Sub
    ...
    Private Function GetTotalCharge(ByVal lngTicketID As Long) As Double
        ...
        Dim decExtraAmount As Decimal 
        
        tkt.Load(lngTicketID)
        decExtraAmount = tkt.AdjustCharge.ToString("C2")
        ...
        While dtr.Read
            lblLaborAmount.Text = FormatCurrency(((FormatCurrency(dtr("AmountCharged"),2)) - (FormatCurrency(decExtraAmount.ToString ,2))),2)
            lblPartsAmount.Text = FormatCurrency(dtr("PartsCharged"),2)
            lblTotalChargeAmount.Text = FormatCurrency(dtr("TotalCharged"),2)
            lblExtraAmount.Text = FormatCurrency(decExtraAmount.ToString,2)
            GetTotalCharge = FormatCurrency(dtr("TotalCharged"),2)
    ...
    Private Sub GetStatuses(lngTicketID As long)
        Dim tkt As New BridgesInterface.TicketRecord(System.Configuration.ConfigurationManager.AppSettings("DBCnn"))
        ...
        tkt.Load(lngTicketID)
        If tkt.TicketClaimApprovalStatusID <> 0 then
          drpStatus.SelectedValue = tkt.TicketClaimApprovalStatusID
    ...
    Private Sub btnAssign_Click(ByVal S As Object, ByVal e As EventArgs)
      Dim strChangeLog As String = ""
            ...
            If drpStatus.SelectedValue <> "Choose One" Then
                tkt.TicketClaimApprovalStatusID = drpStatus.SelectedValue
            End If
            
            
      Dim tnt As New BridgesInterface.TicketNoteRecord(System.Configuration.ConfigurationManager.AppSettings("DBCnn"))
      
      tnt.Add(_ID, Master.WebLoginID, Master.UserID, "Claim Status Has been set to: " & drpStatus.SelectedItem.ToString & " by " & Master.UserName )
      tnt.CustomerVisible = True
      tnt.PartnerVisible = False
      tnt.SourceID = BridgesInterface.TicketNoteRecord.Sources.Customer
      tnt.Acknowledged = False
      ...
      Response.Redirect("InvoicedTicket.aspx?id=" & _ID)  
            ...
            <td class="band" rowspan="2" style="width:1%">
              <div class="bandheader">
                <table style="width: 100%">
                  <tbody>
                    <tr>
                      <td  style="width: 1%" class="label">Assign Claim Status:</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              <div>&nbsp;</div>                            
              <div>&nbsp;</div>
              <div  id="secStatus" class="inputformsectionheader">Choose Status</div>
              <div class="inputform">                
                <div><asp:DropDownList style="width: 99%;" ID="drpStatus" runat="server" /></div>
                <div style="text-align: right;"><asp:Button ID="btnAssign" runat="server" Text="Assign" OnClick="btnAssign_Click"/></div>
              </div>
              <div>&nbsp;</div>
              <div class="inputformsectionheader">Total Charges</div>
              <div class="inputform"> 
                <div>&nbsp;</div>
                <table>
                  <tr>
                    <td><b>Labor:</b></td>
                    <td style="text-align: right;"><asp:Label ID="lblLaborAmount" runat="server" /></td>
                  </tr>
                  <tr>
                    <td><b>Extra:</b></td>
                    <td style="text-align:right;"><asp:Label ID="lblExtraAmount" runat="server" /></td>
                  </tr>
                  <tr>
                    <td><b>Parts:</b></td>
                    <td style="text-align: right;"><asp:Label ID="lblPartsAmount" runat="server" /></td>
                  </tr>
                  <tr>
                    <td><b>Total:</b></td>
                    <td style="text-align: right;"><asp:Label ID="lblTotalChargeAmount" runat="server" /></td>
                  </tr>
                </table>
              </div>
              <div>&nbsp;</div>
